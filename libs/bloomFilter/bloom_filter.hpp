...
public:
    ...
    std::map<std::string, int>* indices = new std::map<std::string, int>;
    ...
    inline std::map<std::string, int>* getKVs()
    {
        return indices;
    }

    inline void insert(const unsigned char* key_begin, const std::size_t& length, std::string value)
    {
    ...
    inline void insert(const T& t)
    {
        ...
        insert(reinterpret_cast<const unsigned char*>(&t),sizeof(T), &t);
    }

    inline void insert(const std::string& key)
    {
        insert(reinterpret_cast<const unsigned char*>(key.c_str()),key.size(),key);
    }
    ...
    inline virtual bool contains(const unsigned char* key_begin, const std::size_t length) const
    {
        ...
        for (std::size_t i = 0; i < salt_.size(); ++i)
        {
            compute_indices(hash_ap(key_begin,length,salt_[i]),bit_index,bit); // TODO: expose the actual indice positions
            if ((bit_table_[bit_index / bits_per_char] & bit_mask[bit]) != bit_mask[bit]) // for the bloomier filter
            {
                return false;
            }
        }
